...
def extract():
    ...
    def extract_fast5(input_file_path, bin_h, mode='DNA'):
        ...
        """
        try:
            (raw_data, raw_label, raw_start, raw_length) = get_label_raw(input_file_path, FLAGS.basecall_group,
    ...
    with open(root_folder,'r') as file:
        while True:
            ...
            if line:
                tmparry = line.split('\t')
                if int(tmparry[3]) >= int(FLAGS.start) and int(tmparry[4]) <= int(FLAGS.end):
                    str_filelist += line+'\n'
                    file_n = tmparry[0]
                    if file_n.endswith('fast5'):
                        if output_file is None:
                            # output_file = open(output_folder + os.path.sep + "data_batch_" + str(batch_idx) + '.bin', 'wb+')
                            output_file = open(output_folder + os.path.sep + os.path.basename(file_n) + '_'+str(batch_idx)+'.bin', 'wb+')
    ...
    if not output_state:
        output_file.close()
        os.remove(output_folder + os.path.sep + os.path.basename(file_n) + '_'+str(batch_idx) + '.bin')
